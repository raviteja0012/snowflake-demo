...
-- Tiny demo seed data. 8 meters, 24 reads (3 per meter, 06:00 to 08:00 UTC).
-- Idempotent via MERGE on natural keys.
-- Demo data only. Real loads come through Validated_Reads_Load_SP / Snowpipe in prod.
...
MERGE INTO FACT_METER_READS tgt USING (
    SELECT * FROM (VALUES
        -- meter 1001 ELEC, kWh rising through morning
        ...
        -- meter 1005 ELEC, one SUSPECT read to show the DQ flag pattern
        ...
        -- meter 1006 GAS, SUSPENDED but still has reads via MDMS catch-up
