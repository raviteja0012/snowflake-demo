...
Mirrors prod AMI SnowSQL config pattern (`ami_env_*.cfg` files with `!define VAR=VALUE`). **Not consumed at runtime** by Snowflake native git integration.

Why it's here:
- Side-by-side readability with the prod AMI codebase
- Reviewers see which variables map to which deploy-time values
- Phase 2 (GitHub Actions + Snowflake CLI) can read these via shell parsing if needed

At deploy time (Phase 1/2), values are passed to the Jinja template through the `USING (...)` clause of `EXECUTE IMMEDIATE FROM`. See `docs/SETUP.md`.

## Mapping cheat sheet

| Prod `ami_env_dev.cfg` | Demo Jinja key       | Demo value                 |
|------------------------|----------------------|----------------------------|
