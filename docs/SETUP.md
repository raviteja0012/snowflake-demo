...
SELECT COUNT(*) AS meters FROM AMI_DEMO_DB.AMISTAGE.DIM_METER;
-- Expect 8

SELECT COUNT(*) AS reads FROM AMI_DEMO_DB.AMISTAGE.FACT_METER_READS;
-- Expect 24

-- Reads joined with meter master, group by service point and type
SELECT m.SERVICE_POINT_ID, m.METER_TYPE_CD, COUNT(*) AS read_count,
...
-- Reads flagged SUSPECT (the one DQ example in the seed)
SELECT m.METER_SERIAL_NBR, r.MEASUREMENT_DATETIME,
       r.READING_QUALITY_CODE, r.MEASUREMENT_QUANTITY
  FROM AMI_DEMO_DB.AMISTAGE.FACT_METER_READS r
  JOIN AMI_DEMO_DB.AMISTAGE.DIM_METER       m USING (METER_KEY)
 WHERE r.READING_QUALITY_CODE = 'SUSPECT';
-- Expect 1 row: MTR-1005 with quantity 0.0
```
